// This is the main code file for the Figma MCP Plugin
...
async function handleCommand(command, params) {
  switch (command) {
    ...
    case "load_font_async":
      return await loadFontAsync(params);
    ...
    case "swap_component":
      return await swapComponent(params);
    ...
    case "set_relative_transform":
      return await setRelativeTransform(params);
    ...
    case "set_paragraph_indent":
      return await setParagraphIndent(params);
    ...
    case "set_reactions":
      return await setReactionsOnNode(params);
    default:
...
async function renameNode(params) {
  ...
  if (!nodeId || !name) throw new Error("Missing nodeId or name parameter");
  ...
  return { id: node.id, name: node.name, oldName };
}

async function setOpacity(params) {
  const { nodeId, opacity } = params || {};
  if (!nodeId || opacity === undefined) throw new Error("Missing nodeId or opacity parameter");
...
async function setVisible(params) {
  ...
  if (!nodeId || visible === undefined) throw new Error("Missing nodeId or visible parameter");
...
async function setLocked(params) {
  ...
  if (!nodeId || locked === undefined) throw new Error("Missing nodeId or locked parameter");
...
async function groupNodes(params) {
  ...
  if (!nodeIds || !Array.isArray(nodeIds) || nodeIds.length < 2) {
    throw new Error("Missing or invalid nodeIds parameter (need at least 2 nodes to group)");
  }
...
async function ungroupNodes(params) {
  ...
  if (node.type !== "GROUP") {
    throw new Error(`Node type ${node.type} is not a GROUP. Only groups can be ungrouped.`);
  }
...
async function insertChild(params) {
  ...
  if (!nodeId || !parentId) throw new Error("Missing nodeId or parentId parameter");
...
async function createLine(params) {
  const {
    x = 0, y = 0, length = 100, rotation = 0,
    strokeColor, strokeWeight = 1, name = "Line", parentId,
  } = params || {};
...
async function createStar(params) {
  const {
    ...
    pointCount = 5, innerRadius = 0.382, name = "Star", parentId,
  } = params || {};
...
async function setCurrentPage(params) {
  ...
  if (!page) throw new Error(`Page not found with ID: ${pageId}`);
...
async function renamePage(params) {
  ...
  if (!pageId || !name) throw new Error("Missing pageId or name parameter");
  ...
  return { id: page.id, name: page.name, oldName };
}
...
// Helper: load font for a text node
async function loadFontForTextNode(node) {
  if (node.type !== "TEXT") throw new Error(`Node type ${node.type} is not TEXT`);
...
// Weight-to-style mapping with common aliases per weight
...
async function loadFontAsync(params) {
  const { fontFamily, fontStyle = "Regular" } = params || {};
  if (!fontFamily) throw new Error("Missing fontFamily parameter");

  await figma.loadFontAsync({ family: fontFamily, style: fontStyle });

  return { fontFamily, fontStyle, loaded: true };
}

async function setFontFamily(params) {
  const { nodeId, fontFamily, fontStyle = "Regular" } = params || {};
  if (!nodeId || !fontFamily) throw new Error("Missing nodeId or fontFamily parameter");

  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  if (node.type !== "TEXT") throw new Error(`Node type ${node.type} is not TEXT`);

  // Load existing fonts first
  ...
  return { id: node.id, name: node.name, fontFamily, fontStyle };
}

async function setFontSize(params) {
  const { nodeId, fontSize } = params || {};
  if (!nodeId || !fontSize) throw new Error("Missing nodeId or fontSize parameter");
  ...
  return { id: node.id, name: node.name, fontSize };
}

async function setFontWeight(params) {
  const { nodeId, fontWeight } = params || {};
  if (!nodeId || fontWeight === undefined) throw new Error("Missing nodeId or fontWeight parameter");
  ...
  // Map weight number to style string, trying multiple candidates
  const candidates = WEIGHT_STYLE_CANDIDATES[fontWeight] || ["Regular"];
  let loadedStyle = null;
  for (const candidate of candidates) {
    try {
      await figma.loadFontAsync({ family, style: candidate });
      loadedStyle = candidate;
      break;
    } catch (e) {
      // This style name doesn't exist for this font, try next
    }
  }
  if (!loadedStyle) {
    throw new Error(`Could not find a font style for weight ${fontWeight} in family "${family}". Tried: ${candidates.join(", ")}`);
  }

  node.fontName = { family, style: loadedStyle };

  return { id: node.id, name: node.name, fontWeight, fontStyle: loadedStyle };
}

async function setTextAlign(params) {
  const { nodeId, horizontal, vertical } = params || {};
  if (!nodeId) throw new Error("Missing nodeId parameter");
  if (!horizontal && !vertical) throw new Error("At least one of horizontal or vertical alignment must be provided");
...
async function setLineHeight(params) {
  const { nodeId, value, unit = "PIXELS" } = params || {};
  if (!nodeId || value === undefined) throw new Error("Missing nodeId or value parameter");
...
async function getStyledTextSegments(params) {
  ...
  const segments = node.getStyledTextSegments([
    ...
    "fills", "textStyleId",
    "fillStyleId", "listOptions",
    "indentation", "hyperlink",
  ]);
...
async function setEffects(params) {
  ...
  if (!nodeId || !effects) throw new Error("Missing nodeId or effects parameter");
  ...
  const figmaEffects = effects.map(effect => {
    ...
    if (effect.type === "DROP_SHADOW" || effect.type === "INNER_SHADOW") {
      return Object.assign({}, base, {
        color: {
          r: effect.color ? effect.color.r : 0,
          g: effect.color ? effect.color.g : 0,
          b: effect.color ? effect.color.b : 0,
          a: effect.color && effect.color.a !== undefined ? effect.color.a : 0.25,
        },
        offset: {
          x: effect.offset ? effect.offset.x : 0,
          y: effect.offset ? effect.offset.y : 4,
        },
  ...
  return { id: node.id, name: node.name, effectCount: figmaEffects.length };
}

async function setBlendMode(params) {
  const { nodeId, blendMode } = params || {};
  if (!nodeId || !blendMode) throw new Error("Missing nodeId or blendMode parameter");
...
async function setGradientFill(params) {
  const { nodeId, gradientType, gradientStops, angle = 0 } = params || {};
  if (!nodeId || !gradientType || !gradientStops) {
    throw new Error("Missing nodeId, gradientType, or gradientStops parameter");
  }
  ...
  // Convert angle to gradient transform (2D affine matrix)
  const radians = (angle * Math.PI) / 180;
  const cos = Math.cos(radians);
  const sin = Math.sin(radians);

  // Gradient handles: start and end points defined by transform
  // Default: left to right (0 degrees)
  ...
  return { id: node.id, name: node.name, gradientType, stopCount: stops.length };
}
...
async function createComponent(params) {
  ...
  if (node.type !== "FRAME" && node.type !== "GROUP") {
    throw new Error(`Node type ${node.type} cannot be converted to a component. Only FRAME and GROUP are supported.`);
  }

  // Create a component from the node
  ...
  // Copy children from the source node
  if ("children" in node) {
    for (const child of [...node.children]) {
  ...
  if ("fills" in node) component.fills = node.fills;
  if ("strokes" in node) component.strokes = node.strokes;
  if ("strokeWeight" in node) component.strokeWeight = node.strokeWeight;
  if ("cornerRadius" in node) component.cornerRadius = node.cornerRadius;
  if ("effects" in node) component.effects = node.effects;
...
async function swapComponent(params) {
  const { instanceId, componentKey } = params || {};
  if (!instanceId || !componentKey) throw new Error("Missing instanceId or componentKey parameter");

  const instance = await figma.getNodeByIdAsync(instanceId);
  if (!instance) throw new Error(`Instance not found with ID: ${instanceId}`);
  if (instance.type !== "INSTANCE") throw new Error(`Node type ${instance.type} is not an INSTANCE`);

  // Import the component by key
  const component = await figma.importComponentByKeyAsync(componentKey);
  if (!component) throw new Error(`Component not found with key: ${componentKey}`);
...
async function getComponentProperties(params) {
  ...
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  if (node.type !== "INSTANCE") throw new Error(`Node type ${node.type} is not an INSTANCE`);
  ...
  return {
    id: node.id,
    name: node.name,
    mainComponentName: node.mainComponent ? node.mainComponent.name : "unknown",
    properties: formattedProperties,
  };
}

async function setComponentProperty(params) {
  const { nodeId, property, value } = params || {};
  if (!nodeId || !property || value === undefined) {
    throw new Error("Missing nodeId, property, or value parameter");
  }
  ...
  // Find the matching property key (Figma uses keys like "Property#1234:56")
  const properties = node.componentProperties;
  let matchedKey = property;
  if (!properties[property]) {
    // Try to find a key that starts with the property name (before the #)
    const found = Object.keys(properties).find(k => k.split("#")[0] === property);
    if (found) matchedKey = found;
  }

  node.setProperties({ [matchedKey]: value });
...
async function booleanOperation(params, operation) {
  const { nodeIds, name } = params || {};
  if (!nodeIds || !Array.isArray(nodeIds) || nodeIds.length < 2) {
    throw new Error("Need at least 2 node IDs for boolean operations");
  }
  ...
  // All nodes must share the same parent
  const parent = nodes[0].parent;
  if (!parent) throw new Error("Cannot perform boolean operation on nodes without a parent");

  // Flatten nodes into the boolean group
  let booleanNode;
  switch (operation) {
    case "UNION":
  ...
  return {
    ...
    operation: operation,
    childCount: booleanNode.children ? booleanNode.children.length : 0,
  };
}
...
async function setRotation(params) {
  ...
  if (!nodeId || rotation === undefined) throw new Error("Missing nodeId or rotation parameter");
...
async function setRelativeTransform(params) {
  const { nodeId, transform } = params || {};
  if (!nodeId || !transform) throw new Error("Missing nodeId or transform parameter");

  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);

  if (!("relativeTransform" in node)) {
    throw new Error(`Node type ${node.type} does not support relativeTransform`);
  }

  // Validate transform matrix shape: [[a,b,tx],[c,d,ty]]
  if (!Array.isArray(transform) || transform.length !== 2 ||
      !Array.isArray(transform[0]) || transform[0].length !== 3 ||
      !Array.isArray(transform[1]) || transform[1].length !== 3) {
    throw new Error("Transform must be a 2x3 matrix: [[a,b,tx],[c,d,ty]]");
  }
...
async function setTextDecoration(params) {
  ...
  if (!nodeId || !decoration) throw new Error("Missing nodeId or decoration parameter");
...
async function setTextCase(params) {
  ...
  if (!nodeId || !textCase) throw new Error("Missing nodeId or textCase parameter");
...
async function setParagraphSpacing(params) {
  ...
  if (!nodeId || spacing === undefined) throw new Error("Missing nodeId or spacing parameter");
...
async function setParagraphIndent(params) {
  const { nodeId, indent } = params || {};
  if (!nodeId || indent === undefined) throw new Error("Missing nodeId or indent parameter");
...
async function setVariableBinding(params) {
  ...
  if (!nodeId || !field || !variableId) {
    throw new Error("Missing nodeId, field, or variableId parameter");
  }
  ...
  // Parse the field path (e.g., "fills/0/color" or "width")
  const parts = field.split("/");

  if (parts.length === 1) {
    // Direct property binding (width, height, itemSpacing, padding*, etc.)
    node.setBoundVariable(parts[0], variable);
  } else if (parts[0] === "fills" && parts.length === 3) {
    // Fill color binding: fills/0/color
    const index = parseInt(parts[1]);
    const fills = node.fills;
    if (fills && fills[index]) {
      node.setBoundVariable("fills", variable, index);
    }
  } else if (parts[0] === "strokes" && parts.length === 3) {
    const index = parseInt(parts[1]);
    node.setBoundVariable("strokes", variable, index);
  } else {
    // Try direct binding as-is
...
async function createImage(params) {
  const {
    imageBytes, x = 0, y = 0, width, height,
    name = "Image", parentId,
  } = params || {};
  if (!imageBytes) throw new Error("Missing imageBytes parameter");

  // Decode base64 to Uint8Array
  const raw = figma.base64Decode(imageBytes);
  const imageHash = figma.createImage(raw).hash;
  ...
  // Resize if dimensions provided, otherwise use a default
  if (width && height) {
    rect.resize(width, height);
  } else {
    rect.resize(200, 200);
...
async function setImageFill(params) {
  ...
  if (!nodeId || !imageBytes) throw new Error("Missing nodeId or imageBytes parameter");
...
async function createVector(params) {
  const { svgPath, x = 0, y = 0, name = "Vector", parentId } = params || {};
  if (!svgPath) throw new Error("Missing svgPath parameter");
  ...
  // Default stroke so the vector is visible
  vector.strokes = [{
    type: "SOLID",
    color: { r: 0, g: 0, b: 0 },
    opacity: 1,
...
async function createPaintStyle(params) {
  ...
  if (!name || !paints) throw new Error("Missing name or paints parameter");

  const style = figma.createPaintStyle();
  style.name = name;

  const figmaPaints = paints.map(paint => {
    if (paint.type === "SOLID") {
      return {
        type: "SOLID",
        color: {
          r: paint.color ? paint.color.r : 0,
          g: paint.color ? paint.color.g : 0,
          b: paint.color ? paint.color.b : 0,
        },
        opacity: paint.color && paint.color.a !== undefined ? paint.color.a : (paint.opacity !== undefined ? paint.opacity : 1),
      };
    }
    // Return as-is for gradient types (requires full gradient config)
    return paint;
  });
...
async function createTextStyle(params) {
  const { name, fontFamily, fontStyle = "Regular", fontSize, lineHeight, letterSpacing } = params || {};
  if (!name || !fontFamily || !fontSize) {
    throw new Error("Missing name, fontFamily, or fontSize parameter");
  }
  ...
  if (lineHeight) {
    if (lineHeight.unit === "AUTO") {
      ...
      style.lineHeight = { value: lineHeight.value, unit: lineHeight.unit };
    }
  }

  if (letterSpacing) {
    style.letterSpacing = { value: letterSpacing.value, unit: letterSpacing.unit };
  }
...
async function createEffectStyle(params) {
  ...
  if (!name || !effects) throw new Error("Missing name or effects parameter");
...
async function setReactionsOnNode(params) {
  const { nodeId, reactions } = params || {};
  if (!nodeId || !reactions) throw new Error("Missing nodeId or reactions parameter");
  ...
  // Build the reactions array matching Figma's format
  const figmaReactions = reactions.map(reaction => {
    const figmaReaction = {
      trigger: { type: reaction.trigger.type },
      actions: [],
    };

    const action = {};
    action.type = reaction.action.type;

    if (reaction.action.destinationId) {
      action.destinationId = reaction.action.destinationId;
    }
    if (reaction.action.navigation) {
      action.navigation = reaction.action.navigation;
    }
  ...
  return {
    id: node.id,
    name: node.name,
    reactionCount: figmaReactions.length,
  };
}
